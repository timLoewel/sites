...
storiesOf('RenderImage', module)
		.addDecorator(getStory => (
				<ScrollView style={{backgroundColor: 'grey'}}>{getStory()}</ScrollView>
		))
		.add('simple', () => (
				<RenderImage
						photoForRendering={{
							...
							description: 'description of this photo which is long, but not too long',
							siteObjectId: 'v8fJe',
							siteName: 'some site name',
							creatorObjectId: 'cud3OhHGcy',
							creatorName: 'Tim Loewel',
							selectedLocation: {address:{formattedAddress: 'Formatted address'}},
							systemLocation:  {address:{formattedAddress: 'system location address'}},
						}}
				/>

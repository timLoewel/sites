...
const RenderImage = React.createClass({
  ...
  snapPhoto: function(
    ...
    targetWidth: number,
    quality: number,
    resultType?: "file" | "base64" | "data-uri"
  ) {
    ...
    const height = t
      ? screenshotDimensions.height * (width / screenshotDimensions.width)
      : undefined;
  ...
  shouldComponentUpdate: function(nextProps, nextState) {
    ...
    const existenceChanged =
      (hadPhoto || willHavePhoto) && !(hadPhoto && willHavePhoto); // xor
    const photosAreDifferent =
      existenceChanged ||
      (hadPhoto &&
        willHavePhoto &&
        this.props.photoForRendering.createdAtMillis !==
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const photosAreDifferent =
      ...
      (hadPhoto &&
        willHavePhoto &&
        this.props.photoForRendering.uriOriginalPhoto !==
          nextProps.photoForRendering.uriOriginalPhoto);
  ...
  _doScreenshot: function(screenshotDimensions) {
    ...
    this.setTimeout(() => {
      this.snapThumbnail(screenshotDimensions)
        .then(thumbnailData => {
          return this.snapPhoto(screenshotDimensions).then(uriPhotoLocal => {
            // both snapshots done
            const photoData = {
              ...this.props.photoForRendering,
              thumbnailData,
              uriPhotoLocal
            };
  ...
  _getLongitudeLatitude: function(textStyle) {
    if (
      this.props.photoForRendering.selectedLocation &&
      this.props.photoForRendering.selectedLocation.longitude
    ) {
      return (
  ...
  _getCreatedAtString: function() {
    return moment(this.props.photoForRendering.createdAtMillis).format(
      "Y MMM d HH:mm"
    );
  },
  ...
  _getAddressString() {
    return (
      ((this.props.photoForRendering.selectedLocation || {}).address || {})
        .formattedAddress || I18n.t("renderImage.addressUnknown")
    );
  },
  ...
  render: function() {
    ...
    const textStyleUnimportant = {
      fontSize: fontSizeUnimportant,
      lineHeight: lineHeightUnimportant,
      color: "grey"
    };
    const textStyleImportant = {
      fontSize: fontSizeImportant,
      lineHeight: lineHeightImportant,
      color: "black"
    };
    const annotationEntry = {
      flexDirection: "row",
      flex: 1,
      paddingBottom: Math.ceil(fontSizeImportant)
    };
    ...
    return (
      <View
        ...
        style={[
          {
            width: this.state.imageLayoutWidth,
            flex: 1,
            flexDirection: "column"
          },
          imageStyles.root
        ]}
        onLayout={this._storeMeasurements}
        collapsable={false}
      >
        <View
          style={{
        ...
        >
          <Text style={[textStyleUnimportant, { color: "black" }]}>
            <Text style={textStyleUnimportant}>
              <Icon
                name="camera"
                style={{ fontSize: this.fontSize, color: "black" }}
              />
            </Text>{" "}
            <Text
              style={{
            ...
            >
              {this.props.photoForRendering.shareableUri}
            </Text>
          </Text>
        </View>
        ...
        <Image
          ...
          style={[
            ...
            {
              width: this.state.imageLayoutWidth,
              height: this.state.imageLayoutHeight
            }
          ]}
          source={{
            uri: "file://" + this.props.photoForRendering.uriOriginalPhoto
          }}
          resizeMode="contain"
          onLoad={this._onLoad}
        />
        <View
          style={[imageStyles.annotations, { padding: textPadding }]}
          collapsable={false}
        >
          ...
          >
            ...
            <Text style={[textStyleImportant, { flex: 1 }]}>
              {I18n.t("renderImage.site", {
                siteName: this.props.photoForRendering.site.name,
                sitePublicUrl: this.props.photoForRendering.site.url
              })}
            </Text>
          </View>
          <View
            style={[annotationEntry, { flexDirection: "column" }]}
            collapsable={false}
          >
            <Text style={textStyleUnimportant}>
...
function bindAction(dispatch) {
  return {
    setReadyForScreenshot: screenshotDimensions =>
      dispatch(readyForScreenshot(screenshotDimensions)),
    setDoingScreenshot: () => dispatch(doingScreenshot()),
    setScreenshotDone: photoData => dispatch(screenshotDone(photoData)),
    errorOnPhoto: error =>
      dispatch(errorOnPhoto({ source: "RenderImage", error }))
  };
}

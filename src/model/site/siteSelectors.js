...
export const getNoSiteSite = state =>
  state.site.sitesByObjectId.get(state.site.noSiteObjectId) ||
  state.site.localSitesByLocalObjectId.get(state.site.noSiteObjectId);

export const getSelectedSite = state =>
  state.site.sitesByObjectId.get(state.site.selectedSiteObjectId) ||
...
export const selectCurrentSite = createSelector(
  ...
  (filter, sites) => {
    ...
    if (
      ...
      if (
        selectedSite &&
        haversineDistance(selectedSite.selectedLocation, filter.location) <
    ...
    if (
      minDistSite &&
      haversineDistance(minDistSite.selectedLocation, currentPosition) <
        MIN_DIST_METERS_FOR_CURRENT_SITE
    ) {
      return minDistSite;
    }

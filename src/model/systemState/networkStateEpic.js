...
const onStoreInitializedEpic = (action$, store) =>
  action$
    .ofType(storeInitialized.getType())
    .map(action => startNetworkMonitor());

/**
 * returns an observable for the netinfo
 */
const getConnectionChangeEventHandler = () =>
...
const initialNetworkstateEpic = action$ =>
  action$
    ...
    .flatMap(action => Observable.fromPromise(NetInfo.fetch()))
    .map(networkState => getConnectionActionFromNetworkState(networkState))
    .catch(error =>
      Observable.of(updateNetworkState({ hasInternet: false, hasWifi: false }))
    );

// monitor network state changes
const monitorNetworkstateEpic = action$ =>
  action$.ofType(startNetworkMonitor.getType()).flatMap(action =>
    getConnectionChangeEventHandler()
      .do(s => {
        console.log(s);
      })
      .map(networkState => {
...
export default combineEpics(
  ...
  initialNetworkstateEpic,
  monitorNetworkstateEpic
);

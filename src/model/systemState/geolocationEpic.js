...
import {
  ...
  updateLocation,
  stopGPS
} from "./systemStateReducer";
import { gpsError, addressError } from "./geolocationReducer";

/**
 * returns an observable for the BackgroundGeolocation event
 */
const getLocationEventObservable = () =>
...
/**
 * returns an observable for the systemState error event
 */
const getLocationErrorEventObservable = () =>
  Observable.fromEventPattern(
    handler => {
 ...
 */
const startBackgroundGeolocationObservable = () =>
 ...
 * starts the background thread
 */
const stopBackgroundGeolocationObservable = () =>
...
const configureBackgroundGeolocation = () =>
  Observable.bindCallback(BackgroundGeolocation.configure)({
    // Geolocation Config
    desiredAccuracy: 0,
    stationaryRadius: 50,
    distanceFilter: 0,
    ...
    stopOnTerminate: true, // <-- do not allow the background-service to continue tracking when user closes the app.
    stopTimeout: 3,
    startOnBoot: false, // <-- no auto start tracking when device is powered-up.
    // HTTP / SQLite config
  ...
  action$
    ...
    .flatMap(action =>
      configureBackgroundGeolocation()
        ...
        .flatMap(state =>
          startBackgroundGeolocationObservable()
            .flatMap(startSuccess =>
              getLocationEventObservable()
                .throttleTime(2000)
...
const addressEpic = action$ =>
  action$
    .ofType(setCurrentLocation.getType())
    .throttleTime(20000)
    .flatMap(action =>
      Observable.fromPromise(
        ...
        .map(addresses => setCurrentAddress(addresses[0]))
        .catch(error => Observable.of(addressError(error)))
    );

export default combineEpics(
  onStoreInitializedEpic,
  startGPSEpic,
  printGeolocationErrorsEpic,
  addressEpic,
  updateLocationEpic,
  addressEpic
);

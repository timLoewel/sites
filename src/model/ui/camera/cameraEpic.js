...
const photographingEpic = (action$: any, store: any): any =>
  action$
    .ofType(photographing.getType())
    .do(v => {
      console.log("photographing Epic");
    })
    .flatMap(photographingAction =>
      Observable.fromPromise(photographingAction.payload.capture)
        .do(v => {
          ...
          Observable.fromPromise(Exif.getExif(photo.path)).flatMap(exif => {
            const newPhoto: IPhoto = {
              ...
              searchableLocation: {
                __type: "GeoPoint",
                longitude:
                  photographingAction.payload.selectedLocation.longitude,
                latitude: photographingAction.payload.selectedLocation.latitude
              },
              selectedLocation: {
                ...photographingAction.payload.selectedLocation
              }, //store.getState().ui.cameraReducer.selectedLocation,
        ...
        .catch(error => {
          console.log(
            photographingAction.payload.createdAtMillis +
              " error in photographing epic"
          );
          console.log(error);
...
const initPhotoLocal = (action$, store) =>
  action$
    .ofType(screenshotDone.getType())
    .do(screenshotDoneAction => {
      RNFetchBlob.fs
        .unlink(screenshotDoneAction.payload.uriOriginalPhoto)
        .catch(err => console.log(err));
    })
    .flatMap(screenshotDoneAction =>
      createUniqueLocalPhotoFilename(
        screenshotDoneAction.payload.createdAtMillis
      )
        .flatMap(newFilePath =>
          Observable.fromPromise(
            RNFetchBlob.fs.mv(
              screenshotDoneAction.payload.uriPhotoLocal.replace("file://", ""),
              newFilePath
            )
          ).flatMap(() =>
            Observable.of(
              addLocalPhoto({
                ...
                siteObjectId:
                  screenshotDoneAction.payload.site.objectId ||
                  screenshotDoneAction.payload.site.localObjectId,
                ...
                searchablePosition:
                  screenshotDoneAction.payload.searchablePosition,

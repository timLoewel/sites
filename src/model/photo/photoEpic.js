...
const batchUploadAllPendingEpic = (action$, store) =>
		action$.ofType(uploadLocalData.getType())
				.filter(() => store.getState().systemState.isConnected)
				.flatMap(uploadLocalDataAction =>
						Observable.from(store.getState().photo.localPhotosByLocalObjectId.toArray())
...
const saveNewFilesToServerEpic = (action$, store) =>
		action$.ofType(addLocalPhoto.getType())
				.map(addLocalPhotoAction => savePhotoFileToServer(addLocalPhotoAction.payload));


// upload one photo to the cloud
const uploadNewLocalPhotoFileEpic = (action$, store) =>
		action$.ofType(savePhotoFileToServer.getType())
				.filter(() => store.getState().systemState.isConnected)
...
export default combineEpics(serverReadyForRequestsEpic, batchUploadAllPendingEpic, saveNewFilesToServerEpic, uploadNewLocalPhotoFileEpic, uploadNewLocalPhotoJsonEpic);